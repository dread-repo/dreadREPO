...
**Decision**: Tier 0 checks: (1) `arch3_try_add_system`: no `TryAddSystem<` outside `DreadSystemInitializer.cs` / `DreadSystemRegistry.cs` (spawn uses registry + `AddComponent(Type)`); (2) `arch3_registry_manifest`: grep `DreadSystemRegistry.cs` for the ten baseline core system type names from [extension-registry.md](./contracts/extension-registry.md).

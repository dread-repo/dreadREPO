...
| `tension-sprint-multiplier` | `Systems/Tension/TensionSystem.cs` | `Traverse` on `SprintSpeedMultiplier` | event (panic sprint / restore) | none | required | required | **keep** | Field not exposed on stub `PlayerController`; panic sprint gameplay |
...
| `dread-init-ui-load` | `Systems/Bootstrap/DreadSystemInitializer.cs` | `EnsureUnityEngineUiLoaded` | startup | none | required | required | **keep** | Defers psychotic break / UI systems until `UnityEngine.UI` loads |
...
| `debug-server-read-player` | `Systems/Debug/DebugServerSystem.cs` | `ReadPlayerFloat` | MCP command | debug server on | required | required | **keep** | Uses Harmony `Traverse`, not raw reflection; version-tolerant member names |
| `debug-server-find-enemies` | `Systems/Debug/DebugServerSystem.cs` | `FindObjectsOfType<EnemyHealth>` | MCP command | debug server on | optional | required | **keep** | Compile-time generic; not reflection |
| `audio-clip-loader-handler` | `Systems/Audio/AudioClipLoader.cs` | `GetDownloadHandlerError` | load | none | required | required | **keep** | Stub `downloadHandler` shape; one-time property lookup per handler type |
| `overlay-texture-format` | `Systems/UI/Shared/OverlayTextureUtil.cs` | `IsFormatUsable` | texture create | none | required | required | **keep** | `SupportsTextureFormat` missing or throws on some Proton builds |
| `monster-overhaul-isplaying` | `Systems/Audio/MonsterOverhaulSystem.cs` | `AudioSource.isPlaying` property | periodic | none | optional | required | **keep** | Fallback if property absent; low frequency |
...
| `plugin-dependency-resolve` | `Systems/Bootstrap/PluginDependencyResolver.cs` | `AssemblyResolve` | startup | none | required | required | **keep** | Load NVorbis etc. from plugin folder |

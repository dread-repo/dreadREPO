...
`scripts/verify-dread.ps1` Tier 0: `arch3_try_add_system` (no stray `TryAddSystem<` outside initializer/registry) and `arch3_registry_manifest` (ten core system types present in `DreadSystemRegistry.cs`, plus three debug types when compiled with `DREAD_DEBUG`).

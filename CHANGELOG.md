...
- **UI-1:** shared `Systems/UI` kit so panels stop reimplementing IMGUI plumbing: `DreadTheme` (single source for the Slate HUD palette), `DreadGui` (shared `EmptyContent`, Proton-safe `SolidTexture`, and `FlatBox`/`Label`/`Button` builders that zero the 9-slice border in one place), and `DreadInputCapture` (cursor unlock + player input lock). Overlay, widgets, toasts, and the error prompt all migrated onto it with no visual change
- **DBG-3:** `DreadFont` resolves a dynamic font from an OS-font fallback chain (monospace-first) so the overlay renders legibly on Proton/Linux instead of blank Arial boxes; applied to every kit label and button. `UnityEngine.TextRenderingModule` is now referenced in all builds (it backs `TextAnchor` / `Font` used by the shared kit)
- **DBG-5:** `DebugOverlayRegistry` lets any feature contribute a foldable, persisted overlay section via `IOverlaySection` (or a title + build delegate), emitting rows through `IOverlayRowSink` with a semantic `OverlayStatus`, without editing `DebugOverlaySystem`
- **DBG-2:** debug overlay background opacity (`DebugOverlayOpacity`, clamped 0.3-1.0), persisted component-kit demo (`DebugOverlayShowKitDemo`), and a forgiving `LoadLayout` that clamps a saved panel position back on-screen after a resolution change
- **Stubs:** `GUI.HorizontalSlider` (plain and styled) and `GUIStyle.fixedWidth` / `fixedHeight` added to the IMGUIModule stub; `RectOffset` + `GUIStyle.border`, `Font` + `GUIStyle.font` / `GUISkin.font`, and a `TextRenderingModule` assembly added so the CI stub build matches the real Unity surface the kit relies on
...
### Fixed
- **MCP bridge (MCP-1):** `dread_get_logs` and `dread_get_patches` text output now reads the field names Unity actually serializes (`Level` / `Message` / `Timestamp`; flat per-patch counts), so text mode no longer prints empty levels, messages, and patch types; `dread_get_state` description matches the real response fields (episode status lives in `dread_get_runtime_state`)
- **Debug server (MCP-2):** requests larger than the documented 4096-byte cap are rejected with `code:-3` before they reach the command queue, enforcing the ADR-0013 limit
- **Core (CORE-2):** `HarmonyPatchCompat.IsMasterClient()` now fails closed: if the `SemiFunc.IsMasterClient` probe is missing or throws (game update, stub mismatch), host-only monster patches stay off instead of running on clients and risking Photon desync; logs one Warning when this happens
- **Core (CORE-1):** forced tumble state in `PlayerTumbleCompat` is keyed per `PlayerController` instance instead of one process-wide flag, so a second consumer can no longer force or release tumble on the wrong avatar
- **Patches (PATCH-1):** plugin unload now sweeps every Dread-owned Harmony hook via `UnpatchSelf` in `PatchLifecycle.Shutdown`, covering the REPOConfig slider label compat that is applied outside the patch registry; the compat can re-apply after a reload
- **Error reporting (ERR-4):** the in-game batch flush now POSTs on a background thread instead of blocking the Unity main thread for up to 15 seconds per send (every captured exception used to trigger a synchronous send); quit and disable drains stay synchronous so reports still survive exit. The payload is also serialized once per send instead of twice
- **Error reporting (ERR-5 / ERR-6):** one Warning per session when the pending error queue is full and new errors are being dropped (previously silent); the dedupe hash map prunes expired entries so long varied sessions stay bounded
- **Error reporting (ERR-7):** when the Worker reports errors it cannot map to a hash, only unsettled reports are re-queued instead of the full batch, so reports GitHub already accepted are not re-sent. Fixed a response-parsing bug where a succeeded report within 256 characters of a failed neighbor was misclassified as failed; response parsing now has xUnit coverage
- **Patches (PATCH-2):** the crouch speed patch honors `CompatibilitySkipConflictingPatches` like the monster patches; the debug-console guard is documented as deliberately exempt (a foreign broken patch is the very thing it guards against)
- **Debug overlay (DBG-6):** FPS/min-FPS sampling stops while the overlay is F10-hidden (PERF-2 follow-up); smoothing resets when shown so readings re-seed within a few frames
- **CI (MCP-4 / DEV-3):** new `mcp` job builds and tests `dread-mcp-server` on every relevant PR
- **Docs (PB-2 / ERR-9 / DOCS-3):** ADR-0011 synced to shipped psychotic-break audio names, hiding condition, partial-class layout, and `ProximityScan`; ADR-0010/0012 transport wording matches the `HttpWebRequest` background-thread flush; duplicate ADR `0007-*` resolved by renumbering the CI pipeline ADR to `0018`
- **MCP bridge (MCP-3):** vitest suite for `dread-mcp-server` (`npm test`): a fixture TCP server exercises `sendCommand` framing, error passthrough, chunked reads, timeouts, and refused/closed connections; formatter tests pin the Unity JSON field names from MCP-1. The TCP client and text formatters moved to `src/tcpClient.ts` / `src/format.ts`; Tier 0 verify runs the suite as `mcp_test`
- **Psychotic break (PB-1):** destroying the system mid-episode (unload, registry teardown) restores player control, re-enables the flashlight, and releases forced tumble; the end-of-episode stumble coroutine is tracked and stopped on scene load so it cannot roll a stale camera, and it bails if the camera is destroyed mid-stumble
